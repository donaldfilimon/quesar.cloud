...
This is the checklist behind "all merged". Source: `donaldfilimon/MLAI-CORPORATION-WWW` at `b6f3686`, from `apps/mlai`, `packages/*`, `apps/quasar`, `sidecars/`, and `docs/`. Spec: `notes/superpowers/specs/2026-09-22-mlai-merge-design.md`.
...
|---|---|---|---|
| `docs/brand.md`, `docs/verification/`, `docs/superpowers/`, dated records | `notes/mlai/` | H | ported `3d35171` |
...
- **Account deletion**: closed 2026-09-22. `user.deleteUser` + `purgeUserData` (authorized `server.ts` edit), and a Delete account card on `/profile`. Earlier text follows for history: account deletion was not offered anywhere in the app. Any future deletion flow must purge the per-user rows in `workspace_connections`, `chat_consents`, `conversation_audits` and `field_notes`, and revoke the Google grant. A foreign key cascade was tried and rejected: auth-off dev mode and the tests use user ids that have no `user` row.
