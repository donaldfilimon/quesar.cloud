...
## Process (superpowers)
1. Brainstorming produces a spec, which is this plan's content written to `quesar.cloud/notes/superpowers/specs/2026-09-22-mlai-merge-design.md`.
2. writing-plans produces `notes/superpowers/plans/2026-09-22-mlai-merge.md`, with a task list per workstream.
...
- Write and commit the spec and plan documents in quesar `docs/superpowers/`. Also write a **gap matrix** at `notes/merge/gap-matrix.md`: every mlai route, API and component, its quesar target, and the owning workstream. This is the checklist that proves "all".
