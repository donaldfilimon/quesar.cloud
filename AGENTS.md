...
| `TRUSTED_PROXY=cloudflare`                                                          | per-IP rate limits and Turnstile read `cf-connecting-ip`. Set only when the origin is reachable solely through Cloudflare (direct `*.vercel.app` access blocked), or callers can forge the header | only `x-real-ip`/`x-forwarded-for` (Vercel-set) are trusted                  |
