...
describe("public research evidence contracts", () => {
  it("keeps all legacy URLs and supplies an overview for each of the six topics", () => {
    ...
    expect(new Set(research.tracks.map((t) => t.id))).toEqual(
      new Set(["ai", "gpu", "mcp", "sea", "tui", "wdbx"]),
    );
    for (const slug of LEGACY)
      expect(research.publications.some((p) => p.slug === slug)).toBe(true);
    ...
    for (const topic of ["gpu", "mcp", "tui"]) {
      expect(
        research.publications.some(
          (p) => p.topic === topic && p.documentType === "implementation-guide",
        ),
      ).toBe(true);
    }
  });
  ...
  it("retains substantive technical support and honest reading times behind short summaries", () => {
    for (const paper of research.publications) {
      const proseWords = paper.body.reduce(
        (n, section) => n + section.paragraphs.join(" ").split(/\s+/).filter(Boolean).length,
        0,
      );
      const technicalAllowance = paper.body.reduce(
        (n, section) =>
          n +
          (section.math?.length ?? 0) * 20 +
          (section.code
            ?.map((c) => c.code)
            .join(" ")
            .split(/\s+/)
      ...
      expect(paper.readTime).toBe(
        `${Math.max(1, Math.ceil((proseWords + technicalAllowance) / 200))} min read`,
      );
    }
    for (const slug of [
      "wdbx-weighted-backtrace-memory-store",
    ...
    for (const paper of research.publications) {
      ...
      for (const source of paper.sources) {
        expect(source.url).toMatch(
          new RegExp(`^https://github\\.com/donaldfilimon/(abi|wdbx)/blob/${source.revision}/`),
        );
      }
    }
    const missingSources = {
      ...research,
      publications: [{ ...research.publications[0], sources: [] }],
    };
    expect(ResearchSchema.safeParse(missingSources).success).toBe(false);
    const unsupportedStatus = {
      ...research,
      publications: [{ ...research.publications[0], status: "Production ready" }],
    };
  ...
  it("binds current and historical PDF links to exact bytes and preserves historical URLs", () => {
    for (const slug of [
      ...
      expect(paper.attachments.find((a) => a.edition === "historical")?.url).toBe(
        `/research/${slug}.pdf`,
      );
