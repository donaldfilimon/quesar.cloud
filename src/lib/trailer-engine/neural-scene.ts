...
export function buildNet3D(
  layers: readonly number[] = NL_LAYERS,
...
function nlDraw(
  ...
  ctx.strokeStyle = "rgba(110,150,210,0.032)";
  ctx.lineWidth = 1;
  ...
  const intro = clamp(t / 6, 0, 1),
    ie = Easing.easeInOutCubic(intro);
  ...
  for (const s of net.stars) {
    ...
    ctx.beginPath();
    ctx.arc(P.x, P.y, 0.6 + P.n * 1.0, 0, 6.3);
    ctx.fill();
  }
  ...
  const np = net.nodes.map((n) => {
    ...
    if (radial > 0.001) {
      x = x * (1 - radial) + n.rx * radial;
      y = y * (1 - radial) + n.ry * radial;
      z = z * (1 - radial) + n.rz * radial;
    }
    const prog = n.li / (L - 1);
    let act = 0;
  ...
  for (const P of np) {
    ...
    if (P.act > 0.45) {
      ...
      g.addColorStop(
        0,
        `rgba(${Math.min(255, hr + 110) | 0},${Math.min(255, hg + 90) | 0},255,${P.act * 0.5 * intensity})`,
      );
      g.addColorStop(1, "rgba(0,0,0,0)");
      ctx.fillStyle = g;
      ctx.beginPath();
      ctx.arc(P.x, P.y, rad * 3.2, 0, 6.3);
      ctx.fill();
    }
    ctx.fillStyle =
      P.act > 0.6
  ...
  vg.addColorStop(1, "rgba(1,2,5,0.66)");
  ctx.fillStyle = vg;
  ctx.fillRect(0, 0, W, H);
}
...
export class NeuralScene implements Scene {
  ...
  draw(ctx: DrawContext, width: number, height: number, t: number): void {
    ...
    o.waves = M.waves;
    o.hue = [
      o.hue[0] + (M.hue[0] - o.hue[0]) * k,
      o.hue[1] + (M.hue[1] - o.hue[1]) * k,
      o.hue[2] + (M.hue[2] - o.hue[2]) * k,
    ];
