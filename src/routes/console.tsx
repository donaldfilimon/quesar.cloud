...
import { JourneyRail, PageClose, PageHero, RouteFrame, Section, Surface } from "@/components/site";
...
export const Route = createFileRoute("/console")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Private field notes on the Quesar architecture, scoped to your account." },
    ],
  }),
...
function ConsolePage() {
  ...
  return (
    <RouteFrame>
      <RequireSession>
        {(user) => <ConsoleInner name={user.displayName ?? user.primaryEmail ?? "operator"} initialNode={node} />}
      </RequireSession>
    </RouteFrame>
  );
}

function ConsoleInner({ name, initialNode }: { name: string; initialNode?: string }) {
  ...
  const node = useMemo(() => architectureNodes.find((item) => item.id === nodeId) ?? architectureNodes[1], [nodeId]);
  ...
  return (
    <>
      ...
      <Section>
        {initialNode ? (
          <p className="mb-6 rounded-lg bg-card px-4 py-3 text-sm text-fg-muted shadow-[var(--shadow-border)]">
            From architecture: this form is attached to <span className="text-fg">{node?.name}</span>. Current versus
            not claimed is listed beside it.
          </p>
        ) : null}
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(16rem,0.7fr)]">
          <form onSubmit={onSubmit} className="grid gap-3 rounded-xl bg-card p-5 shadow-[var(--shadow-border)]">
            <div>
          ...
          {node ? (
            <Surface>
              <div className="flex items-center justify-between gap-3">
                <p className="font-mono text-[0.68rem] tracking-[0.14em] text-accent uppercase">{node.name}</p>
                <StatusBadge status={node.status} />
              </div>
              <p className="mt-3 text-sm text-fg-muted">{node.detail}</p>
              <p className="mt-4 font-mono text-[10px] tracking-[0.14em] text-status-current uppercase">Current</p>
              ...
              <Separator />
              <p className="font-mono text-[10px] tracking-[0.14em] text-fg-subtle uppercase">Not claimed</p>
              <ul className="mt-1 space-y-1 text-sm text-fg-muted">
        ...
        <ul className="mt-6 space-y-3">
          {status === "loading" ? <li className="text-sm text-fg-muted">Loading your notes…</li> : null}
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                          ...
                          <AlertDialogAction onClick={() => void onDelete(note.id)}>Delete</AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                  </div>
      ...
      <PageClose
        ...
        secondary={[{ to: "/console/workspace", label: "Workspace" }]}
        next={[{ to: "/profile", label: "Profile", body: "Account, sign-out, and where notes live." }]}
      />
    </>
  );
}
