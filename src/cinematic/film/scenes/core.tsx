  ...
  const tokenEls = QUERY_TOKENS.map((tok, i) => {
    ...
    return (
      <span
        key={i}
        style={{
          color: isHot ? C.cyan : C.text,
          textShadow: isHot ? `0 0 16px ${C.cyan}88` : "none",
          borderBottom: isHot ? `2px solid ${C.cyan}` : "2px solid transparent",
          paddingBottom: 2,
        }}
      >
        {shown}
    ...
    <SceneBox>
      ...
      <div
        style={{
          ...
          zIndex: 20,
          opacity: queryIn,
          transform: `translateY(${(1 - queryIn) * 16}px)`,
        }}
      >
        <div
          style={{
        ...
        >
          {tokenEls}
          <span style={{ opacity: typed < 1 ? (Math.sin(lt * 8) > 0 ? 1 : 0) : 0, color: C.cyan }}>
            ▎
          </span>
        </div>
      </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                gap: 14,
                marginBottom: 16,
                opacity: step(lt, 4.2 + i * 0.35, 0.5),
              }}
            >
              <span
                style={{
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: 7,
                    background: p.accent,
                    boxShadow: isWin ? `0 0 14px ${p.accent}` : "none",
                  }}
                />
              </div>
              <span
      ...
      <DiagramSVG>
        <Elbow
          pts={elbowPts}
          draw={wireWin}
          color={ROUTE_PERSONAS[winnerIdx]!.accent}
          width={2.5}
        />
        ...
        return (
          <div
            ...
            style={{
              position: "absolute",
              left: cardX,
              top: cardTop + i * (cardH + cardGap),
              width: cardW,
              height: cardH,
              zIndex: 16,
              opacity: rv,
              transform: `translateX(${(1 - rv) * 26}px) scale(${1 + glow * 0.02})`,
              borderRadius: 18,
              border: `1px solid ${isWin ? p.accent + "aa" : C.line}`,
              background: isWin
                ? `linear-gradient(150deg, ${p.accent}1f, rgba(10,10,16,0.92))`
                : "rgba(255,255,255,0.025)",
              boxShadow: glow
                ? `0 24px 60px rgba(0,0,0,0.5), 0 0 40px ${p.accent}33`
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              <div
                style={{ fontFamily: FONT.display, fontWeight: 700, fontSize: 32, color: C.text }}
              >
                {p.name}
              </div>
              <div
        ...
        {VEC_PTS.map((p, i) => {
          ...
          return (
            <circle
              ...
              cy={p[1]}
              r={isNN ? 6 : 3.4}
              fill={isNN ? C.cyanHi : C.dim2}
              opacity={op}
              style={isNN ? { filter: `drop-shadow(0 0 7px ${C.cyan})` } : undefined}
            />
          );
        })}
        <circle
          ...
          r={10 * fieldIn}
          fill={C.blue}
          style={{ filter: `drop-shadow(0 0 14px ${C.blue})` }}
        />
        {fieldIn > 0.5 && (
          <PulseRing
            cx={QUERY_PT[0]}
        ...
        return (
          <div
            ...
            style={{
              ...
              zIndex: 16,
              opacity: rv,
              transform: `translateY(${(1 - rv) * 20}px) scale(${1 + flash * 0.03})`,
              borderRadius: 16,
              border: `1.5px solid ${borderC}${flash > 0.1 ? "ff" : "88"}`,
              background:
                flash > 0.1
                  ? "rgba(40,12,12,0.9)"
                  : "linear-gradient(160deg, rgba(20,20,30,0.95), rgba(8,8,14,0.95))",
              boxShadow:
                flash > 0.1
                  ? `0 0 36px ${C.red}66`
                  : ok
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{ fontFamily: FONT.mono, fontSize: 13, color: C.dim2 }}>
            ...
            <div style={{ marginTop: 4 }}>
              ...
              <div style={{ fontFamily: FONT.mono, fontSize: 16, color: C.dim }}>
                {i === 0 ? "genesis" : hexOf(i + 10) + "…"}
              </div>
            </div>
            <div>
              <div style={{ fontFamily: FONT.mono, fontSize: 12, color: C.dim2 }}>hash</div>
              <div style={{ fontFamily: FONT.mono, fontSize: 16, color: ok ? C.green : b.accent }}>
                {hexOf(i + 11) + "…"}
              </div>
            </div>
            {ok && (
      ...
      {reject > 0.05 && verify < 0.6 && (
        <div
          style={{
            ...
            width: bw,
            zIndex: 24,
            opacity: reject * (1 - verify),
            transform: `translateY(${(1 - reject) * -8}px)`,
            textAlign: "center",
          }}
        >
          <div
