...
const VERDICTS: readonly VerdictRow[] = [
  [
    "Typography refresh",
    "already-strong",
    "A Major-Third modular scale (h00→h5) and a Space Grotesk / IBM Plex Sans / IBM Plex Mono split already ship. We tune leading & tracking rather than rebuild.",
  ],
  [
    "Visual depth / glass",
    "already-strong",
    "glass-card, layered noise and blur are in place. The gap isn't more glass — it's consistent light direction and a real elevation ramp.",
  ],
  [
    "Premium noise globally",
    "already-strong",
    "Shipping at opacity 0.015 with mix-blend overlay. Keep as-is; no change needed.",
  ],
  [
    "Inconsistent spacing",
    "real-gap",
    "The most honest line in the spec. Solved with a spacing scale + rhythm tokens, not utility-by-utility guesswork.",
  ],
  [
    "Two-step surfaces",
    "real-gap",
    "bg → surface is only two tiers; cards-on-cards muddy. A 4-tier elevation ramp fixes depth system-wide.",
  ],
  [
    "Color split",
    "real-gap",
    "Brand board runs a 6-color spectrum; the site runs one blue. They must reconcile into action vs. identity vs. semantic roles.",
  ],
];
  ...
  return (
    <section>
      ...
      <Glass
        ...
        className="mt-6 p-7"
        style={{
          background: "linear-gradient(145deg,rgba(34,211,238,0.06),rgba(168,85,247,0.04))",
        }}
      >
...
function ColorGroup({
  ...
  return (
    <div>
      ...
      <div
        className={`grid gap-4 ${big ? "grid-cols-2 sm:grid-cols-3" : "grid-cols-2 sm:grid-cols-4"}`}
      >
        {items.map((it) => (
          <Swatch key={it.token} name={it.token} value={it.value} note={it.note} big={big} />
        ))}
      </div>
    </div>
  );
          ...
          {PERSONA.map((p) => (
            <Glass key={p.name} level={2} className="p-5 relative overflow-hidden">
              <div className="relative flex items-center gap-3">
                <span
                  className="w-9 h-9 rounded-xl flex items-center justify-center font-bold text-black/80"
                  style={{ background: p.color, boxShadow: `0 0 18px -2px ${p.color}` }}
                >
                  {p.name[0]}
                </span>
                <div>
  ...
  return (
    <section>
      ...
      <div className="grid lg:grid-cols-[1.1fr_1fr] gap-8 items-start">
        <div
          className="rounded-3xl p-8 sm:p-12 flex items-center justify-center"
          ...
          {ELEVATION.map((e) => (
            ...
            >
              <div
                ...
                style={{
                  background: e.bg.length > 7 ? e.bg.slice(0, 7) : e.bg,
