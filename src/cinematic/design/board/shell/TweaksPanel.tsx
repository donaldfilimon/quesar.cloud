/* ════════════════════════════════════════════════════════════════
   ...
   __edit_mode_set_keys / __edit_mode_dismissed).
...
export function TweaksPanel({
  ...
  useEffect(() => {
    const onMsg = (e: MessageEvent<EditModeMessage>): void => {
      const t = e.data?.type;
  ...
  const onDragStart = (e: ReactMouseEvent<HTMLDivElement>): void => {
    const panel = dragRef.current;
    if (!panel) return;
...
export function TweakRadio({
  ...
  const segAt = (clientX: number): string => {
    ...
    const clamped = opts[Math.max(0, Math.min(n - 1, i))];
    return clamped ? clamped.value : valueRef.current;
  };
...
function TwkCheck({ light }: { light: boolean }): ReactNode {
  return (
