...
export function SessionsCard({
  ...
  const others = state.kind === "ready" ? state.sessions.filter((s) => s.id !== currentSessionId) : [];
  return (
    <Surface>
      <p className="font-mono text-[0.68rem] tracking-[0.16em] text-fg-subtle uppercase">Active sessions</p>
      {state.kind === "loading" ? <p className="mt-3 text-sm text-fg-muted">Loading sessions…</p> : null}
      {state.kind === "stale" ? (
      ...
      {state.kind === "ready" ? (
        <ul className="mt-3 grid gap-3">
          {state.sessions.map((session) => {
            ...
            return (
              <li key={session.id} className="flex flex-wrap items-start justify-between gap-3 border-t border-border pt-3">
                ...
                {current ? (
                  <span className="font-mono text-[10px] tracking-[0.14em] text-status-current uppercase">This device</span>
                ) : (
                  <Button type="button" variant="secondary" size="sm" disabled={busy} onClick={() => onRevoke(session.token)}>
                    Revoke
                  </Button>
                )}
              </li>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Button type="button" variant="secondary" disabled={busy || others.length === 0} onClick={onRevokeOthers}>
