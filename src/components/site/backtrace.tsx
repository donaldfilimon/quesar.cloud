  ...
  return (
    <figure className={cn("rounded-lg bg-bg-elevated shadow-[var(--shadow-border)]", className)}>
      ...
      <div className="relative px-5 py-5 sm:px-6">
        <span className="chain-draw absolute top-7 bottom-9 left-[1.6rem] w-0.5 bg-accent sm:left-[1.85rem]" aria-hidden="true" />
        ...
        <ol className="relative mt-3 grid gap-4">
          {rows.map((row) => (
            <li key={row.id} className="grid grid-cols-[1.25rem_minmax(0,1fr)] gap-2.5">
              <span
                className={cn("relative mt-1.5 size-2.5 justify-self-center rounded-full ring-4 ring-bg-elevated", row.dot)}
                aria-hidden="true"
              />
              <div className="min-w-0">
                <div className="flex items-baseline justify-between gap-3">
                  <span className="truncate font-mono text-xs text-fg-muted">{row.src}</span>
                  <span className={cn("font-mono text-xs tabular-nums", row.text)}>{row.weight.toFixed(2)}</span>
                </div>
                ...
                <div className="mt-2 h-1 rounded-full bg-muted" aria-hidden="true">
                  <i className={cn("block h-full rounded-full", row.dot)} style={{ width: `${row.weight * 100}%` }} />
                </div>
                <p className="mt-1.5 font-mono text-2xs text-fg-subtle">
                  {row.id} <span aria-hidden="true">←</span>
                  <span className="sr-only">parent</span> {row.parent} <span className="sr-only">, via {row.persona}</span>
                </p>
              </div>
            </li>
          ))}
