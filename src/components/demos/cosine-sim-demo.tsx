...
export function CosineSimDemo() {
  ...
  const col = cos > 0.5 ? "var(--status-current)" : cos > -0.1 ? "var(--warn)" : "var(--destructive)";
  ...
  return (
    <div className="surface grid items-center gap-6 p-6 sm:grid-cols-2">
      ...
      <div>
        ...
        <label htmlFor={idA} className="mb-1 block text-xs text-accent">Vector a — {a}°</label>
        <input
          id={idA}
        ...
        />
        <label htmlFor={idB} className="mb-1 block text-xs text-accent">Vector b — {b}°</label>
        <input
          id={idB}
